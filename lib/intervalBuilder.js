    ...
    CheckEvent.find()
        ...
        .find(function(err, checkEvents) {
            ...
            checkEvents.forEach(function(checkEvent) {
                // console.log("is it true?:" + self.changeObjectState(checkEvent.check, checkEvent.message));
                if (self.changeObjectState(checkEvent.check, checkEvent.message)) {
                    // console.log("is it true_2?:" + self.updateCurrentState(checkEvent.timestamp));
                    self.updateCurrentState(checkEvent.timestamp);
                }
            });
            // console.log("is it true?:" + self.completeCurrentInterval(end));
            ...
            if (typeof callback === "function") {
                // self.completeCurrentInterval(end);
                callback(self.completeCurrentInterval(end));
            }
        });
};
