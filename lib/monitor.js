...
Monitor.prototype.pollChecksNeedingPoll = function(callback) {
  ...
  this.findChecksNeedingPoll(function(err, checks) {
    ...
    checks.forEach(function(check) {
      self.pollCheck(check, function(err, body, res) {
...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  var pollerCallback = function(err, time, res, pollerDetails) {
    ...
    if (err) {
      return self.createPing(err, statusCode, check, now, time, pollerDetails || details, callback);
    }
    try {
      self.emit('pollerPolled', check, time, res, pollerDetails || details);
      self.createPing(null, statusCode, check, now, time, pollerDetails || details, callback);
      
    } catch (error) {
      return self.createPing(error, statusCode, check, now, time, pollerDetails || details, callback);
    }
  };
  try {
    ...
    return self.createPing(incorrectPollerUrl, undefined, check, now, 0, details, callback);
  }
...
Monitor.prototype.createPing = function(error, statusCode, check, timestamp, time, details, callback) {
  ...
  var req = http.request(options, function(res) {
    ...
    res.on('end', function() {
      if (callback) callback(null, body, res);
    });
  }).on('error', function(e) {
