var uptimeBar = (function(){
  ...
  var uptimeBarCheck = function(args) {
    ...
    if (from < firstChecked && firstChecked < to) {
      ret += '<div style="background-color:#fff;left:0;width:' + (firstChecked - from) / duration * 100 + '%"></div>';
    }

    // hide not yet measured period
    if (from < now && now < to) {
      // hide not yet measured period
      ret += '<div style="background-color:#fff;right:0;width:' + (to - now) / duration * 100 + '%"></div>';
    }
    ret += '</div>';
