...
describe('populateFromDirtyCheck', function () {
    ...
    it('should update object', function () {
        ...
        assert.strictEqual(checkWithName.name, 'new-name');
        assert.strictEqual(checkWithName.url, 'http://www.new-url.fr');
    });

    it('should not erase empty values', function () {
        checkWithName.populateFromDirtyCheck({url: 'http://www.new-url.fr'}, pollerCollection);
        assert.strictEqual(checkWithName.name, 'name1');
        assert.strictEqual(checkWithName.url, 'http://www.new-url.fr');
    });
    ...
    it('should use given url as name if check has no name and no name is given', function () {
        ...
        assert.strictEqual(checkWithoutName.name, 'http://www.new-url.fr');
        assert.strictEqual(checkWithoutName.url, 'http://www.new-url.fr');
    });
    ...
    it('should use given name if check has no name name is given', function () {
        ...
        assert.strictEqual(checkWithoutName.name, 'new-name');
        assert.strictEqual(checkWithoutName.url, 'http://www.url2.fr');
    });
});
